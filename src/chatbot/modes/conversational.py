...
class ConversationalMode:
    ...
    async def process(
        ...
        retrieval_results = await self.rag.retrieve(
            query=message,
            top_k=5,
            filters=filters
        )
        ...
        response = await self.rag.generate(
            ...
            conversation_history=self._get_recent_history(session_id)
        )
        ...
        if intent in ['quote', 'buy', 'apply', 'purchase']:
            suggested_action = {
                ...
                'flow': 'quotation',
                'buttons': [
                    {'label': 'Yes, get a quote', 'action': 'start_guided_quotation'},
                    {'label': 'No, just browsing', 'action': 'continue_chat'}
                ]
            }
            ...
            suggested_action = {
                ...
                'products': [
                    self._generate_product_card(p[2])
                    for p in products
                ]
            }
    ...
    def _get_recent_history(self, session_id: str, limit: int = 5) -> List[Dict]:
        ...
        # Get from PostgreSQL
        messages = self.state_manager.db.get_conversation_history(
    ...
    def _generate_product_card(self, product: Dict) -> Dict:
        ...
        return {
            ...
            'actions': [
                ...
                {'type': 'get_quote', 'label': 'Get a Quote'}
            ]
        }
