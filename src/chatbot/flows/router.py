...
class ChatRouter:
    ...
    async def route(
        ...
        if self._is_exit_intent(message) and session.get('mode') == 'guided':
            ...
            return {
                'message': '👋 Exited guided flow. How else can I help you?',
                'mode': 'conversational'
            }
    ...
    def _is_guided_trigger(self, message: str) -> bool:
        ...
        triggers = [
            'quote', 'buy', 'apply', 'purchase', 'get insurance',
            'how much', 'price', 'cost', 'premium',
            'i want', 'i need', 'help me choose'
        ]
    ...
    def _is_exit_intent(self, message: str) -> bool:
        ...
        exits = [
            'exit', 'cancel', 'stop', 'go back',
            'start over', 'nevermind', 'not now'
        ]
