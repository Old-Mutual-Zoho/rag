...
class PaymentFlow:
        ...
        if current_step == 0:  # Payment method selection
            ...
            if requires_agent:
                return {
                    'response': {
                        'type': 'agent_required',
                        'message': f'💼 For premiums above UGX {self.AUTO_PAYMENT_THRESHOLD:,}, we\'ll connect you with an agent to guide you through the payment process.',
                        'agent_info': {
            ...
            payment = self._create_payment_record(
                quote=quote,
                payment_method=collected_data['payment_method'],
            ...
            if collected_data['payment_method'] == 'mobile_money':
                payment_result = await self._initiate_mobile_money_payment(
                    payment_details,
                    premium_amount
                )
    ...
    async def _initiate_mobile_money_payment(self, payment_details, amount):
        ...
        # For now, return mock
        return {
            'transaction_ref': f"MM{uuid.uuid4().hex[:10].upper()}",
            'status': 'pending'
        }
